...
{{-- ROLE TABS --}}
<div class="tabs">
    <a href="{{ route('attendance.index') }}" class="tab {{ request('role') == null ? 'active' : '' }}">All</a>
    <a href="{{ route('attendance.index', ['role' => 'intern']) }}" class="tab {{ request('role') == 'intern' ? 'active' : '' }}">Interns</a>
    <a href="{{ route('attendance.index', ['role' => 'employee']) }}" class="tab {{ request('role') == 'employee' ? 'active' : '' }}">Employees</a>
</div>
...
<td><span class="badge {{ $att->status }}">{{ ucfirst(str_replace('_',' ',$att->status)) }}</span></td>
<td>{{ $att->location ?? '-' }}</td>
